...
- Frameshift 2.0 and legacy Elite Trader installations can update in place;
  both executable names remain supported by this release.
- The first 2.1 launch may spend extra time rebuilding commander history from
  local journals. The migration is resumable, keeps preserved user-entered
  specialist data, and does not prevent live game-process detection.
- No settings reset or manual database conversion is required. Frameshift
  validates migrated storage before promotion and retains recovery backups.
